...
class _ConnectionScreenState extends State<ConnectionScreen> {
  ...
  getConnection() async {
    Map response = await apiBaseHelper.getAPICall(Uri.parse(baseUrl + "userConnectionInfo/$curUserId")); //todo change this user id
    setState(() {
      loading = false;
    });
  ...
  acceptConnection(id, status1) async {
    ...
    if (isNetwork) {
      try {
        ...
        Map response = await apiBaseHelper.postAPICall(Uri.parse(baseUrl + "connectionRequestAcceptORReject"), data);
        print(response);
        bool status = true;
        setState(() {
          acceptStatus = false;
  ...
  sendConnection(sendId, receiveId, PeopleModel model) async {
    await App.init();
    isNetwork = await isNetworkAvailable();
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        backgroundColor: Color(0xffF2F4F8),
        elevation: 1,
        title: text("Connections".toString(), textColor: MyColorName.primaryDark, fontFamily: fontMedium, fontSize: 14.sp),
        actions: [
          InkWell(
            onTap: () {
            ...
            children: [
              ...
              GestureDetector(
                ...
                child: Container(
                  ...
                  child: Center(
                    child: text("Manage Connection", textColor: MyColorName.colorTextPrimary, fontFamily: fontRegular, isLongText: true, fontSize: 12.sp),
                  ),
                ),
              ),
              boxHeight(30),
              Row(
                ...
                children: [
                  Expanded(
                    child: GestureDetector(
                      ...
                      child: Container(
                        ...
                        child: Center(
                          child: text("Request", textColor: MyColorName.colorTextPrimary, fontFamily: fontRegular, isLongText: true, fontSize: 12.sp),
                        ),
                      ),
                    ),
                  ),
                  ...
                  Expanded(
                    child: GestureDetector(
                      onTap: () async {
                        await navigateScreen(context, ConnectScreen(connectionList.toList()));
                        getConnection();
                      },
                      child: Container(
                        height: getHeight(90),
                        ...
                        child: Center(
                          child: text("Connection", textColor: MyColorName.colorTextPrimary, fontFamily: fontRegular, isLongText: true, fontSize: 12.sp),
                        ),
                      ),
                    ),
                  ),
              ...
              requestList.length > 0
                  ? Container(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          text("Request".toString(), textColor: MyColorName.primaryDark, fontFamily: fontRegular, fontSize: 10.sp),
                          ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                return ListTile(
                                  ...
                                  leading: ClipRRect(borderRadius: BorderRadius.circular(100), child: commonImage(model.profileImage, 100.0, 100.0, "", context, "")),
                                  title: text(getString(model.name.toString()), textColor: MyColorName.primaryDark, fontFamily: fontMedium, fontSize: 10.sp),
                                  trailing: !acceptStatus
                                      ? Row(
                          ...
                          Center(
                            child: InkWell(
                              ...
                              child: text("Show More".toString(), textColor: MyColorName.colorTextPrimary, fontFamily: fontBold, fontSize: 10.sp, isCentered: true),
                            ),
                          ),
                        ],
                      ),
                  ...
                  : SizedBox(),
              boxHeight(30),
              peopleList.length > 0 ? text("People you may know".toString(), textColor: MyColorName.primaryDark, fontFamily: fontRegular, fontSize: 10.sp) : SizedBox(),
              boxHeight(15),
              peopleList.length > 0
                  ? ListView.separated(
                            ...
                            child: Container(
                              ...
                              child: Center(
                                child: !model.connectionStatus
                                    ? text(model.requestStatus == "1" ? "Sent" : "Connect",
