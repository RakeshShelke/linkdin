...
class _ConnectScreenState extends State<ConnectScreen> {
            ...
            children: [
              Container(
                ...
                child: Row(
                  ...
                  children: [
                    text("${widget.connectionList.length ?? 0} Connections".toString(), textColor: MyColorName.colorTextPrimary, fontFamily: fontMedium, fontSize: 10.sp),
                    /* Image.asset(
                      "images/search.png",
                                ...
                                children: [
                                  Expanded(
                                    child: Column(
                                      crossAxisAlignment: CrossAxisAlignment.start,
                                      children: [
                                        Text(getString(model.name.toString()), style: textStyle14px500w),
                                        boxHeight(6),
                                        text(getString(model.email.toString()), textColor: Color(0xff6D6D6D), fontFamily: fontRegular, fontSize: 7.5.sp),
                                  ...
                                  Column(
                                    children: [
                                      Icon(Icons.remove_circle_sharp, color: MyColorName.colorIcon).onClick(() async {
                                        apiController
                                            .get<RemoveConnectionResponse>("https://developmentalphawizz.com/Nerdmine/webService/deleteConnectionRequest/$curUserId/${model.userId}")
                                            .then((RemoveConnectionResponse response) {
                                          if (response.status ?? false) {
                                            FlutterToastX.showSuccessToastBottom(context, "User is removed from connections");
                                      ...
                                      boxHeight(10),
                                      Icon(Icons.chat, color: MyColorName.primaryDark).onClick(() {
                                        callChat(model);
                                      }),
                                    ],
                                  ),
  ...
  Future<void> getConnectInfo() async {
    ...
    UserConnectionInfoResponse response = await apiController.get<UserConnectionInfoResponse>("${EndPoints.USER_CONNECTION_INFO}/$userId");
