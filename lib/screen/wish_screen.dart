  ...
  savedJob(id) async {
    ...
    if (isNetwork) {
      try {
        ...
        Map response = await apiBaseHelper.getAPICall(Uri.parse(baseUrl + "removeItemFromWishlist/${id}"));
        print(response);
        bool status = true;
        setState(() {
          saved = false;
              ...
              if (tempPostWishList.isNotEmpty)
                !loading
                    ? ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          FeedModel jwl = FeedModel.fromJson(tempPostWishList[index].toJson());
                          return InkWell(
                            onTap: () async {
                              navigateScreen(context, PostDetail(jwl, []));
                              // var result = await Navigator.push(context, MaterialPageRoute(builder: (context) => PostDetail(FeedModel.fromJson(jwl.toJson()), [])));
