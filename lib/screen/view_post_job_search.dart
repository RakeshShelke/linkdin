...
class _ViewPostJobSearchState extends State<ViewPostJobSearch> {
  ...
  getJobs() async {
    ...
    if (response['status']) {
      ...
      jobList.addAll(jobAndApplicantsResponse.jobList ?? []);

      for (JobList v in jobAndApplicantsResponse.jobList ?? []) {
        JobModel jobModel = JobModel.fromJson(v.toJson());
        jobModel.totalApplicant = v.totalApplicant;
        v.jobType == "Government job" ? listOfGovJob.add(jobModel) : listOfPvtJob.add(jobModel);
      }
    }
  ...
  deleteJob(id) async {
    ...
    if (isNetwork) {
      try {
        ...
        Dialogs.showLoader(context, "Deleting job");
        Map response = await apiBaseHelper.postAPICall(Uri.parse(baseUrl + "deactivateJob"), data);
        print(response);
        bool status = true;
        setState(() {
          saved = false;
                ...
                child: !loading
                    ? ListView.separated(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return InkWell(
                            onTap: () {
                              navigateScreen(context, JobDetailScreen(jobModel.jobId.toString(), listOfAppliedJobPvt: jobModel.totalApplicant ?? []));
                            },
                            child: Container(
                                  ...
                                  Expanded(
                                    ...
                                    children: [
                                      ...
                                      text("Your profile match this job",textColor: Color(0xff5e5e5e),fontFamily: fontRegular,fontSize: 7.5.sp),
                                        ...
                                        boxHeight(10),
                                        text(getDuration(jobModel.createDate ?? ""), textColor: MyColorName.primaryDark, fontFamily: fontMedium, fontSize: 7.sp),
                                      ],
                                    ),
                                  ),
                                  boxWidth(10),
                                  Column(
                                    children: [
                                      if (App.localStorage.getString("user_type_id") == "6" || App.localStorage.getString("user_type_id") == "7")
                                        InkWell(
                                            onTap: () {
                                              deleteJob(currentTabJobs[index].jobId);
                                              // setState(() {
