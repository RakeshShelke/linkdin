...
import 'package:nerdmine/model/remove_connection_response.dart';
...
import '../model/get_course_details.dart';
...
class ApiController {
  ...
  Future  postV2(String url, {Map<String, dynamic>? headers, body, encoding, payload}) async {
    // Map<String, dynamic> headersMap = {"Authorization": await AuthUser.getInstance().token()};
    // if (headers != null) headersMap.addAll(headers);
...
class JsonConverter {
  static T fromJson<T>(dynamic value) {
    if (T == PostLineItemBottomSheetResponse) return PostLineItemBottomSheetResponse.fromJson(value) as T;
    if (T == PostCreatedResponse) return PostCreatedResponse.fromJson(value) as T;
    if (T == ConnectCountResponse) return ConnectCountResponse.fromJson(value) as T;
    if (T == UserConnectionInfoResponse) return UserConnectionInfoResponse.fromJson(value) as T;
    ...
    if (T == AddExperienceReponse) return AddExperienceReponse.fromJson(value) as T;
    if (T == SkillListResponse) return SkillListResponse.fromJson(value) as T;
    if (T == NotificationResponse) return NotificationResponse.fromJson(value) as T;
    if (T == UploadImgResponse) return UploadImgResponse.fromJson(value) as T;
    if (T == PostJobPicklistResponse) return PostJobPicklistResponse.fromJson(value) as T;
    if (T == SearchResponse) return SearchResponse.fromJson(value) as T;
    if (T == PostCounsellingResposne) return PostCounsellingResposne.fromJson(value) as T;
    if (T == ResumeUploadResponse) return ResumeUploadResponse.fromJson(value) as T;
    if (T == AddCoachResponse) return AddCoachResponse.fromJson(value) as T;
    if (T == ExpertiesResponse) return ExpertiesResponse.fromJson(value) as T;
    if (T == GetCompanyProfile) return GetCompanyProfile.fromJson(value) as T;
    if (T == UpdateBioInformation) return UpdateBioInformation.fromJson(value) as T;
    if (T == AddExperienceReponse) return AddExperienceReponse.fromJson(value) as T;
    if (T == AddBioResponse) return AddBioResponse.fromJson(value) as T;
    if (T == GetCoursesResponse) return GetCoursesResponse.fromJson(value) as T;
    if (T == JobSearchResponse) return JobSearchResponse.fromJson(value) as T;
    if (T == AddExperienceReponse) return AddExperienceReponse.fromJson(value) as T;
    if (T == GetCourseDetails) return GetCourseDetails.fromJson(value) as T;
    if (T == RemoveConnectionResponse) return RemoveConnectionResponse.fromJson(value) as T;
